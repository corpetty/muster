...
- **Delivery node stood up in the app (2026-08-24).** `delivery_module` (v0.2.0, the repo the demo consumes) is now a declared dependency of `muster_module` (the `lp_*` caller) and `muster_ui`, and it rides the standalone runner's module set (`make build` bundles it via the builder's `collectAllModuleDeps`). Verified by launching the runner offscreen: the host logs `Module loaded: delivery_module` → `DeliveryModuleImpl: Initialized successfully`, so `coordinate_join`'s `lp_client_create("delivery_module")` now resolves instead of raising. `publish()` records to the local log before broadcasting (`session.nim`), so a single instance's propose/contribute/fold completes without a peer. **Not yet verified on this box:** that `coordinate_join` boots the node (`createNode`/`start`) and the loop renders end-to-end — the standalone app is GUI-only (no headless invoke) and this machine has no working GL to drive the UI (the standing UI-e2e blocker); a driveable host closes it. The basecamp bake used by `ui/tests` would separately need delivery bundled to run the room loop there.
- **Live two-instance test over a real delivery node** — delivery embeds its own Waku node; two hosts on separate networks through store nodes; kill mid-collection, restart, contribution still lands (R-4/R-6). Needs a running node **and** a network config (the join today boots with `"{}"`, an isolated node — fine single-instance, but two-instance needs the endpoint set, invariant 8).
