...
- **Membership/grant handshake** — two hosts establishing a shared epoch key over the wire (today `coordinate_join` founds a fresh single-member epoch; the grant flow is exercised only in-process in the tests). Pairs with the live test.
