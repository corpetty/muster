...
module/          Nim core behind the muster.lidl contract → muster-module.lgx
  ...
  src/wallet/    chain-agnostic wallet (F-Design): types (Account/AssetId/Amount over stint UInt256) · adapter (ChainAdapter seam + Wallet aggregate) · evm_adapter (ETH + ERC-20 over user RPC · verifiedBalance via eth_getProof · client-side signing for any node) · evm_sign (nim-eth EIP-155 tx signing via the keystore seam) · mock_chain (non-EVM: shielded accounts, delayed finality) · verify (nim-eth verifyMptProof — the Nimbus verified-proxy core, in-process, F-10)
...
**Chain-agnostic wallet (F-Design) — landed 2026-08-23.** A `ChainAdapter` seam + `Wallet` aggregate (`module/src/wallet/`) with two impls proving genericity: an EVM adapter (ETH + ERC-20 over the user's RPC, inv 8 / F-10) and a mock shielded chain modelled on the LEZ (dual public/shielded accounts, delayed finality, sentinel-failure defense). Amounts are arbitrary-precision (wei > uint64). Failure is always a raise — a failed read never reads as a zero balance. Hosted methods: `wallet_accounts`/`wallet_balances`/`wallet_estimate_fee`/`wallet_send`/`wallet_finality`. Real LEZ adapter deferred (needs the zone's proving stack); client-side EVM RLP signing deferred (needs a live node — refused explicitly, not faked).
