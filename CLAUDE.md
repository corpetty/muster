...
- **Hosted coordination surface** — the remaining lidl methods to drive a conversation from a host (start/join, contribute), wiring `coordination/intents.nim` into `muster_module` (replacing the `musterP3LinkCheck` stub). `identity()` is the first piece landed; the coordination stack is still tested standalone, not yet driven by a host.
