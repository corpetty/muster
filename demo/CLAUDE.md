...
- **Conversation state is a fold, never a second record.** The journey, the home surface and the proposals are all `reduce(messages)` over `get_messages`, so they cannot drift, cost nothing to rebuild, and are correct for a conversation this instance did not start. Two passes where order matters — store-node catchup delivers an approval before its proposal often enough to matter. This used to claim the fold "survives a restart for nothing", which was false: `chat_module` keeps identity and conversations in memory only, so after a restart `get_messages` has nothing to fold and the peer has a new address entirely. That is upstream and deliberate for now — a regression from the DeMLS pivot, tracked at `logos-messaging/libchat#28` — see `poc/BUG-chat-module-state-not-persisted.md`. The consequence for us: **a prepared demo state can carry the wallet but never the conversation.**
