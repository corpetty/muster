...
- **ADR-013 UI build chain — decouple `muster-ui` from the module's Nim-cdylib builder** — proposed (P4 unblock, 2026-08-21). Adopted as the working path to a rendering, real-engine UI **now**, while `exo-c6a`'s upstream fix lands on its own clock. Reversible: when the upstream codegen bump ships, re-evaluate returning to the single-builder chain.
