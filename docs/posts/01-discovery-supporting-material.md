...
- **Contracting** — the stage where the agreement is *formalized*: for a chain action, building the transaction that gets signed and then signing it; off-chain, whatever the formality of agreement is. The matrix looks at one facet of it — whether the signing and execution context stays local and verifiable, or a web frontend introduces ambient leakage — but the full stage also pulls in the infrastructure and dependencies the agreement needs and the *commitments* each party is making (imperative vs motivational; see post 3). Muster reads contracting in that wider sense.
...
| Contracting | ◑→● | Local-first by construction: a native client, no web frontend, no ambient leakage. **The signing context *is* now verifiable** — F-4 re-derivation is built in the core (the client re-derives the materialization from the displayed effect and refuses to sign on mismatch, cannot be disabled), the correction post 2 documents; a user reviews a re-derived payload, not a bare amount. Residual: the byte-level provenance commitment and the refuse-on-unaccountable gate are tested-but-unwired on the shipping Safe rail (post 2), and *which type of commitment each party holds* — and, for motivational ones, the defection cost — is not yet surfaced (`exo-3ae`, post 3) |
