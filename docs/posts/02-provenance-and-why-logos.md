...
should carry an account of where its inputs came from. The concrete claim I'll defend is
narrower than the title and, I think, still worth the post: **for every signing decision,
Muster shows you a per-input, class-and-position lineage — graded by how strongly the code can
actually vouch for each entry — folded from the encrypted conversation log and put in front of
you at the moment you approve.** The stronger form (that lineage bound into the signed bytes,
with a hard refusal on anything unaccountable) is specified and tested but not yet on a shipping
signing path — and I'll be exact about that line rather than blur it. Research post; argue with
it.*
...
[F-20](../01-furps.md). Stated plainly:
...
**"Commits to."** The record is not metadata stapled to the signature. It is folded
...
**"Refuses."** By design, not a warning and not a yellow badge: when any input's origin
...
And the record has to be **two-way complete**: every input that reached the bytes has an
...
into test failures. Be clear on the limit, though: the check is over the inputs the code
*registers* as reaching the payload. It proves the record is internally faithful to that set —
not that the set itself caught everything that ever influenced the effect. An input that never
enters the log is outside the guarantee, the way a type system is honest about the values it
sees and silent about the ones you never handed it.

### What of this actually ships, said flatly

Here is the honest seam, and I'd rather put it right under the mechanism than bury it. **The
byte-commitment and the refuse-on-unaccountable gate — the two properties just described — are
tested core primitives
...
structure the Safe contract fixes (it binds the environment and the call, but leaves no field
for a Muster provenance record). So *within the Safe standard* the commitment can't live in the
owner's signature; it can only ride alongside, in the log. The byte-level version becomes real
only for a driver where Muster owns the payload end to end — which is design, not deployment,
today. That has a consequence worth stating against my own thesis: until the commitment is *in*
the signature, the refusal is a property of an honest client checking itself, not something a
counterparty or a chain can verify happened. Run a stock Safe client and the guardrail is
simply absent; the resulting signature is indistinguishable. For self-protection that's still
worth a lot. As a third-party-verifiable guarantee it isn't there — and on Safe, not without
leaving the Safe signing standard.

What *does* ship is the rest of it — the accountable, graded, epoch-scoped lineage, folded from
the log and put in front of you at decision time — and that's the part the rest of the post is
really about. It's less than the title. It's also, as far as I can find, a thing no wallet
currently puts in front of you, and I'll place it carefully against what they do.
...
itself**: rebuildable from log plus keys, nothing on the side to drift (invariant 4;
...
reconstructs none of an epoch's record (`probe_provenance_epoch_scope_stepper`;
...
hidden in the UI, not merely unshown, *absent from the record* (the model check asserts a
...
and settlement live in one client over one conversation log — the shipping core wraps each
epoch key as a libsodium sealed box (ECIES over X25519) to each member's encryption identity,
with secp256k1 as the bound authorization key ([F-16](../01-furps.md); ADR-010; native MLS is
the post-v0 target, not what runs today) — and the driver seam that lets provenance respect
anonymity is the same seam that lets the room's capabilities grow by proposal. That assembly is what "it lets me put
...
SLSA** sign statements about where each input to a build came from — the closest prior art, a
widely-adopted framework and specification. **Sigstore / Rekor** and Certificate Transparency
are append-only signed logs with *third-party-verifiable* inclusion proofs — stronger, on the
verifiability axis, than Muster-on-Safe, precisely because they're publicly checkable. **C2PA**
signs content provenance for media. Content-addressed DAGs (Git, IPFS) make lineage structural.

And closer to home, on the wallet itself: EIP-712 domain separation already binds *what* you
sign to a chain and contract, and transaction simulators (Blockaid, Tenderly) already show you
the *predicted effect* and flag known-bad targets before you sign. So "no wallet does anything
here" would be false, and I don't claim it. But those answer different questions than this one:
a simulator predicts what the transaction *does*; it does not produce a per-input record of
*where each input came from*, classed and positioned, bound to the conversation the decision
happened in. Effect-prediction and input-provenance are complementary, not the same axis.
...
- **The core** (`module/`, the Nim client) implements all six clauses of `derived-exo-3a1`,
  ...
  criticality on the spec is *catastrophic*. A green oracle here means the mechanism is correct,
  not that it's on a path a user reaches — for the two commitment/refusal clauses that
  distinction is the whole of the caveat above, and for the other four the mechanism and the
  shipping lineage are the same code.
...
The line between what ships and what's still a primitive is the one drawn above (*What of this
actually ships*), and I won't re-derive it — the graded lineage is live; the byte-commitment
and the hard refusal are tested but off the signing path. The one thing to add here is what
none of the three builds touches: the transport-metadata leak (FS-9), which sits underneath all
of them because it isn't theirs to close.
...
- **"Commits to"** → [`mech-signing-payload.svg`](../diagrams/mech-signing-payload.svg) —
  *"What a signing payload commits to, and what each field defeats"* (F-5/FS-3/F-20). By the
  "commits to" discussion — caption should note it draws the *specified* payload; the shipping
  seam stays in prose.
...
> step 7) is currently red on `main` (`exo-eba`) — and which side is wrong, the code or a
> drifted test expectation, is still an open call on that ticket. Resolve it green before the
> shot runs; a credibility built on "claims trace to a passing test" can't rest its one live
> proof on a red one.*
