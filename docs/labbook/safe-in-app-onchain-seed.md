...
## The full 2-of-3 on-chain run (needs anvil + a running delivery node)

1. `infra/anvil/devnet.sh` — anvil + the MiniSafe at `0x5FbDB…` funded, owners 0/1/2.
2. Two peers, each seeded with a different owner key and pointed at the same RPC + a
   delivery node:
   ```
   MUSTER_DEV_SECP_KEY=0xac09…ff80 make run PEER=alice   # alice = owner 0
   MUSTER_DEV_SECP_KEY=0x59c6…690d make run PEER=bob     # bob   = owner 1
   ```
3. Both join the same room; alice proposes a Safe payment; **each clicks Approve** —
   two in-app owner signatures fold to executable (2-of-3), no paste.
4. Alice clicks **Settle on-chain** → `coordinate_submit` assembles the `execTransaction`
   from the two folded owner signatures, submits through the RPC, and reads finality
   from the receipt. `checkSignatures` passes because both signers are real owners.

Single instance, no second peer: seed as owner 0, Approve in-app (owner 0), then
**Paste a signature instead** with owner 1's signature (the advanced fallback) → 2-of-3
executable → Settle.
