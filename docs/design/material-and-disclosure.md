...
| **K6 surface — landed 2026-09-16** offers/share lidl + handlers | Done: `coordinate_offers`/`compose_offers`/`coordinate_share_material` in muster.lidl + handlers + `moduleCatalogue()`; the offers JSON payload (`offersPayload`) is verified by `offers_test`. Needs `tools/regen.sh` + a host build to wire the generated dispatch. |
| **K6 UI** composer step three, card "From you", LEZ Mode B | Host-build: Room.qml loads `coordinate_offers`→`card.offers`; MusterCard renders the picker + calls `coordinate_share_material`; Composer's third step from `compose_offers`; LEZ Mode B as a room proposal with a counterparty address slot. Launch-verified in the runner; the offscreen self-test asserts the offers payload. |
