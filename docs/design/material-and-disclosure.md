...
- **contributor** → the contribution. `coordinate_contribute(intent, keyRef)` signs with the chosen key. The room learns which key signed, as it does today (every driver is named, ADR-015).
...
F-9 derives each member's role by intersecting chat membership with account membership. With offers, that intersection is per proposal: for the live intent, each member is *needed* (a slot is theirs), *supplied* (their `material-share` or contribution is in the log), *declined* (`coordinate_decline`), or *not needed*. The scope panel shows this, naming each member's room identity; slots fill as contributions arrive (U-4). No member ever sees another's catalogue or candidates: the log carries what was supplied, never what could have been.
...
| `module/src/coordination/intents.nim` | **Landed (K5 core):** `materialShareEvent` + `reduceShares` (folds one per requirement×sharer, inv 4; only the public face + class travel, s1/s2); `logProvenance` classes a share `peer-message` (F-20), naming the sharer (every driver is named, ADR-015); `reduceFlow` discloses the shared field to the room, outside rows waiting for submit (s6). `materialshare_test` green. Keyed contribute + per-key F-14 binding ride K2b. |
...
| `ui/src/qml/Composer.qml` | **Landed (exo-fa1), build-verified:** the F-18 third step for the "pay" verb — the account (the room's shared Safe), the asset+amount picker fed by `compose_offers` (my holdings; the Safe driver declares the amount as a proposer asset slot bound to `value`; the in-room propose composer bounds the amount by the Safe's live balance — **can't over-send**, exo-bf9), and the destination (typed, or "ask the room" → the counterparty request). Confirm drafts the first intent (`enterRoom`→`proposeInRoom`). Module + UI `.lgx` build; offscreen startup clean. |
...
| **K3 — landed; anvil e2e run green 2026-09-16** Verified authority: Safe owners from chain; self-injection removed | `readiness` reports `safe-owner: missing` for a non-owner and `unknown` with no RPC (`readiness_test`); the owner set is read from the chain (`safe_owners_test`); the self-injection is gone. **Live run:** `safe_anvil_e2e` (2-of-3 → on-chain, recipient +1 ETH) + `coordinate_submit_anvil` (room-side settle, twice at the live nonce) both green against a fresh anvil MiniSafe. |
...
| **K5** Binding events: `material-share`, keyed contribute, per-key binding in-room | A counterparty share folds onto the request, the composed effect's provenance names it at its position, `coordinate_flow` shows the rows it added, and the proof (`coordinate_proof`) covers it, naming the sharer. |
...
3. **Anonymous drivers and counterparty material — resolved by ADR-015 (2026-09-23).** Anonymous membership is retired: every driver is named, so a `material-share` is a disclosure by a named member to the room, and the privacy boundary is the room, not the signer. Reintroducing anonymous membership would be a deliberate relaxation behind a new ADR, and would reopen this question.
