...
| `module/src/api/muster.lidl` | `compose_offers()` (candidate actions × proposer slots), `coordinate_offers(intent_id)` (this proposal's slots that are mine), `coordinate_share_material(intent_or_request_id, requirement, handle)`, `coordinate_contribute` gains a key handle. Generated surface via `tools/regen.sh`. |
| `ui/src/qml/Composer.qml` | The F-18 third step, fed by `compose_offers`; the menu greys rather than hides. |
| `ui/src/qml/MusterCard.qml` | "From you" in the needs box: slot → candidates → disclosure rows → pick. |
...
| **K2b** Keyed keystore | `Keystore` grows `accounts(): seq[KeyRef]` + `sign(ref, …)`/`edSign(ref, …)`/`bindingFor(ref, …)`; `FileKeystore` becomes a keyfile set (open question 1: keyfile set first); `sign(ref)` refuses an unknown ref; the host source stub reports `unknown` until ADR-013's shell. Ripples through the signing path — one coherent change. |
...
| **K6** UI: composer step three, card "From you", LEZ Mode B | Launch-verified in the runner: propose from a chosen account, the peer sees the slot that is theirs with its disclosure rows, picks, and the intent completes; the offscreen self-test asserts the offers payload. |
