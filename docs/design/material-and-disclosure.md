...
| `ui/src/qml/Composer.qml` | **Landed (exo-fa1), build-verified:** the F-18 third step for the "pay" verb — the account (the room's shared Safe), the asset+amount picker fed by `compose_offers` (my holdings; the Safe driver declares the amount as a proposer asset slot bound to `value`), and the destination (typed, or "ask the room" → the counterparty request). Confirm drafts the first intent (`enterRoom`→`proposeInRoom`). Module + UI `.lgx` build; offscreen startup clean. |
