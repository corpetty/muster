...
**Status:** design + plan, 2026-09-15. Epic `exo-002` (pebbles). M1 (`exo-002.1`) landed 2026-09-15; M2 (`exo-002.2`), M3 (`exo-002.3`), M8 (`exo-002.8`), M4 (`exo-002.4`), M5 (`exo-002.5`) and M6 (`exo-002.6`, SDK PR #3) landed 2026-09-16; M7's muster half too. Remaining: M7's upstream half (Basecamp).
...
| **M3** Card: five questions + per-participant readiness + install / authorize / approve / deny — **landed** (`MusterCard.qml` needsBox + Deny; module `coordinate_decline`, `decline_test`) | `exo-002.3` | M2 | A proposer sees the peer's missing dependency on the card; the peer can deny. Launch-verified in the runner. |
| **M4** Provenance for all actions: messages/admits/drops carry an input record; epoch-scoped exportable proof — **landed** (`logProvenance` over every event kind, `coordinate_provenance`; admits are log events, closing `exo-275`; `log/proof.nim` + `coordinate_proof` / `coordinate_verify_proof`, tamper refuses) | `exo-002.4` | (`exo-275`) | Proof verifies iff the log is unchanged; tamper refuses. Out-of-room proof scope decided and documented. |
| **M5** Information-flow view: fold log × disclosure × epoch membership into a per-action observer matrix — **landed** (`coordination/flow.nim` `reduceFlow` + `observerMatrix`, `coordinate_flow`, `FlowView.qml` in the room; `flow_test`) | `exo-002.5` | M1, M4 | Walkthrough's static claims and the derived matrix agree on every step for a Safe intent; the derived view names the store node. |
| **M6** `lidl-gen driver` emits manifest declarations as a third declared input — **landed** (logos-nim-sdk PR #3: `[declarations.json]`, validated against the closed vocabulary, `declared:false` when absent, Tier-1 skeleton carries the `manifest` override; muster pin bumped to `9aa82f3`) | `exo-002.6` | M1 | Emitted manifest `nim check`s clean and passes consistency; `metadata.json` rev bumped. (SDK repo.) |
| **M7** Host hook: allowed effects keyed on capability name; dispatch refused without a matching executable intent — **muster's half landed** (`intents/authorization.nim`, `coordinate_authorization` / `coordinate_check_authorization`, `authorization_test`; proposal `docs/design/host-effect-policy.md` + `infra/access-policy.effects.example.json`). **Upstream half open:** the broker consulting the grant before dispatch. | `exo-002.7` | M6 | Proposal doc + policy schema drafted here; raised with Basecamp. muster's half: `coordinate_authorization(intent)`. (Upstream.) |
