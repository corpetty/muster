...
| Facts | Nothing represents `capability_module`, the `ui-host` process split, or the Logos Messaging node embedded in delivery | Correct as-is — this is a **layer** diagram, not a **process** diagram. Those facts belong to `arch-instance-topology`. Keep the two from blurring |
...
Thirty-one entries in four families. Each names what it shows, who it serves, what it derives from, and what it must not claim. Entries marked **▣** are views of a shared interactive substrate (§3.4) rather than standalone figures.
...
| **pipeline-private-transfer-flow** | *(exists)* The keystone's **flow companion**: one private→private payment walked down the real stack — `chat_module`+`delivery_module` for the room and the address exchange, `lez_core` for the shielded value — as six steps (open room · ask · share key · agree · pay · settle), each marking what leaves the device and who learns it. Where the keystone tables stage against infrastructure, this traces the payment through it, making "the architecture diagram is the privacy diagram" literal for the post's second placeholder | **BOTH** | `ChatBackendWallet.cpp` (the method path); `demo/CLAUDE.md`; `GAPS.md`; `arch-instance-topology` for the box names | Ordering/mempool stays **not assessed**; the step-4 flag and the footer draw F-4/FS-6 and F-20 as specified-not-built, never as properties the demo has. The `~7-minute` prove and settle-poll are one machine's measurement — §5's "do not state as typical" applies |
...
| **mech-effect-materialization** | F-4/FS-6: what the user reviews (the effect, semantic), what gets signed (the materialization, bytes), the client re-deriving the second from the first, and refusing on mismatch. The contrast with blind signing is the whole point | **BOTH** | `CLAUDE.md` invariant 1; F-4, FS-6 | **Built** (P1, `reviewAndCheck`; live in the P4 UI) — the demo still has no such check, which is why `GAPS.md` scores Contracting ◑ there. Since F-20, one overclaim remains available and must be avoided: a match is not sufficient to offer a signature, so do not draw this check as the last gate — point at `mech-provenance-trail` |
| **mech-signing-payload** | F-5 as a drawn record: environment, account, slot, expiry, `profile`, `schema-id`, effect value root, and the F-20 provenance record. Why the signature is worthless anywhere else, and what each field defeats | POST | `CLAUDE.md` invariants 2, 5b and 10; ADR-009 | That `profile` + `schema-id` are payload fields — they are not yet. The other rows are built (P1, live since exo-ef1) and each row says which. The demo builds no signing payloads at all |
| **mech-provenance-trail** | F-20/FS-10: that F-4's re-derivation proves a materialization agrees with its effect and says nothing about where the effect's *inputs* came from — so correctly-derived data of unknown origin passes it. Then the record that closes it (class + log position, per input class), the refusal that gives it teeth, and the two boundaries it does and does not defend | **BOTH** | `CLAUDE.md` invariant 10; F-20, FS-10; spec `derived-exo-3a1` | **Built** — all six probes exist and the path is live (`attest.nim`, exo-ef1). Do not draw it as covering a signature made outside muster: that approval carries no attestation and is graded *unattested*. Nor draw it as hiding lineage from participants — it does not, and cannot: the boundary is the room |
| **mech-reduce-log** | State = reduce(log): append, ingest-verify, causal order with deterministic tiebreak, idempotent reducers, convergence under reorder and duplication. Worth drawing the adversarial-order property test as part of the figure | BUILD | `CLAUDE.md` invariant 4; R-2, R-3; P0 accept | That every event is author-signed. F-1 asks for it; the build signs only the events that name an author (a message, a decline, a material share, an account disclosure, a FROST join — exo-f76), and an approval carries a signature its driver verifies. The rest are content-addressed, hash-linked and sealed under the epoch key, with no signature of their own. And the demo has **no log** — it folds `get_messages`. Same shape, different substrate, and the difference is what "nothing you could replay to prove what was agreed" means |
| **mech-membership-epochs** | F-16: epoch re-key on add and on remove, and precisely what a new member cannot read. The UI seam depends on this being real | **BOTH** | `CLAUDE.md` invariant 7; ADR-002 | That these are MLS epochs. ADR-010 resolved to our own sealed-box stopgap behind `ConversationCrypto`, with native chat's MLS as the target; the demo "neither verifies nor claims" re-keying |
| **mech-driver-seam** | Invariant 6 drawn: the core never interprets contribution bytes. Rounds, serialization domain and finality are driver-described. Stub / Safe / FROST against one interface, with the conformance suite as the gate | BUILD | `CLAUDE.md` invariant 6; F-6, F-7; P1–P2, P6 | That F-6's seven methods sit on one interface — the build splits them across the Driver, the core's `Collection` and the Settlement seam. Nor that F-8's visible queue exists |
| **mech-two-identities** | ADR-010's blocking issue: an Ed25519/Curve25519 chat identity and a secp256k1 settlement key, holding the *same principle* on *different curves*, with no authenticated binding between them — and F-9 needing exactly that binding to derive per-member role. A live open decision, which makes it a better figure than a settled one | **BOTH** | ADR-010; `GAPS.md` §3 "nothing binds the chat identity to the zone account" | Drawn first as an **unresolved** problem; ADR-010's revisit (2026-08-23) resolved F-14 to two bound keypairs, and the figure now draws that decision — do not draw the one-curve ideal as surviving |
| **mech-plugin-blocks** | F-12/F-13: a plugin's manifest, its declared capabilities, content-hash verification at load, and the single narrow thing it may do — emit typed blocks. No canvas, no keys, no network | BUILD | `CLAUDE.md` invariant 3; P5 | Part built: only the P1 sandbox and its inv-3 boundary. The runtime, signed bundles and the lying-plugin test (P5's accept criterion) are not current properties, and the sandbox's block kinds are not F-13's six |
...
**`mech-provenance-trail` is a third exception, on the `mech-two-identities` reasoning rather than the `mech-intent-lifecycle` one.** Its value is that it draws a *hole* — that F-4's re-derivation is satisfied by correctly-derived data of unknown origin — and a hole is worth drawing before its fix is built, not after. It shipped with a status banner harder than any other figure's, because F-20 was then the least discharged thing in the repo: specified, no probes written, and four of six oracles waiting on an executor that did not exist. That stopped being true — all six probes exist and exo-ef1 wired the path live — and the banner now says **built**.
...
| | |
|---|---|
| `arch-contract-seam.svg` | The two codegen paths and the small hand-written region each ends in, with the two rules `lidl_c.h` states outright and ADR-008's fallback. First drawn with the Nim backend as not existing, because it did not; it now shows `logos-nim-sdk`, and the view half is the real `ui/` (`muster_ui.rep`, `MusterUiBackend`) rather than the demo's |
...
| | |
|---|---|
| `pipeline-private-transfer-flow.svg` | One private→private payment traced down the stack in six steps, each hop marked with what leaves the device and who — the store node or the zone — learns it. The flow companion to the substrate-pipeline keystone, and the figure for the post's "private to private txn within the Logos stack" placeholder |
...
| `mech-effect-materialization.svg` | **built · inv 1** — F-4, FS-6 · P1, live at P4. Amended for F-20: a match is drawn as one gate passing, not as the signature being offered |
| `mech-signing-payload.svg` | **specified · part built** — F-5, FS-3, F-20 · P1, live since exo-ef1. Each row is marked; `profile` + `schema-id` are not yet payload fields |
| `mech-provenance-trail.svg` | **built · inv 10** — F-20, FS-10 · all six `derived-exo-3a1` probes, live in `attest.nim` (exo-ef1); a signature made outside muster grades unattested. Shipped as the weakest of this column — no probes, four oracles awaiting an executor — and redrawn once that stopped being true |
| `mech-reduce-log.svg` | **built · inv 4** — R-2, R-3, S-2 · P0, multi-party P3. Author-naming events are signed (exo-f76); F-1's signature on every event is not, and the strip says so |
| `mech-membership-epochs.svg` | **built · F-16** — P3: X25519 sealed-box epochs behind `ConversationCrypto` (ADR-010 resolved) |
| `mech-driver-seam.svg` | **built · inv 6** — F-6, F-7, S-1 · P1–P2. F-6's seven methods are drawn with where the build puts each; F-8's queue is not built |
| `mech-two-identities.svg` | **resolved · ADR-010** — F-14, F-9. The revisit chose two bound keypairs |
| `mech-plugin-blocks.svg` | **specified · part built** — F-12, F-13, FS-5 · the sandbox at P1; the runtime, bundles and lying-plugin test at P5, tagged on the rows |

Written against `01-furps.md` directly rather than from the summaries, so every id and every quoted property is the normative text. `mech-two-identities` began as the one to keep: F-14 was marked *Contested* in the requirements themselves, so the figure drew an acknowledged open decision and three ways out without picking one — until ADR-010's revisit picked one, and the figure was redrawn to it.

**Re-audited 2026-09-25 against the code.** Commit 6430973 (2026-08-21) had flipped only the `<desc>` and footer text of several of these to built, leaving the visible pill still saying *specified · not built* — and the pill is the part a screenshot keeps. The pills now match the code, each strip names what is and is not built, and the figures drawn after their subject existed (`mech-wallet-seam`, `mech-verified-reads`, `mech-driver-contract`, `arch-nim-reuse`) carried a **built** pill from the start.
