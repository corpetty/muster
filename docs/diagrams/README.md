...
| **pipeline-settlement-bug** | The account-id derivation drawn: id = f(viewing key, identifier); the published keys carry no identifier; the sender's client picks one at random; the note lands at an address the recipient has never seen. A worked example of a leak-class **nobody chose** | POST | `demo/poc/BUG-private-transfer-recipient-identifier.md`; `GAPS.md` §4b | Should show the workaround (sum over enumerated accounts) and that it is a workaround |
