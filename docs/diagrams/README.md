...
| **mech-driver-seam** | Invariant 6 drawn: the core never interprets contribution bytes. Rounds, serialization domain and finality are driver-described. Stub / Safe / FROST against one interface, with the conformance suite as the gate | BUILD | `CLAUDE.md` invariant 6; F-6, F-7; P1–P2, P6 | — |
...
| `mech-effect-materialization.svg` | **specified · not built** — F-4, FS-6 · P4. Amended for F-20: a match is drawn as one gate passing, not as the signature being offered |
| `mech-signing-payload.svg` | **specified · not built** — F-5, FS-3, F-20 · P1 |
| `mech-provenance-trail.svg` | **specified · not built, and unchecked** — F-20, FS-10 · P0–P1. Weaker than the rest of this column: `derived-exo-3a1` passed the SPECIFY gate, but none of its six probes exist and four of its oracles are `model_check` against an executor that has not landed |
