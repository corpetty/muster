...
| **mech-reduce-log** | State = reduce(log): append, ingest-verify, causal order with deterministic tiebreak, idempotent reducers, convergence under reorder and duplication. Worth drawing the adversarial-order property test as part of the figure | BUILD | `CLAUDE.md` invariant 4; R-2, R-3; P0 accept | That events are author-signed. F-1 asks for it, but an event is content-addressed, hash-linked and sealed under the epoch key, with no per-author signature (exo-f76); only an approval carries a signature, which the driver verifies. And the demo has **no log** — it folds `get_messages`. Same shape, different substrate, and the difference is what "nothing you could replay to prove what was agreed" means |
...
| `mech-reduce-log.svg` | **built · inv 4** — R-2, R-3, S-2 · P0, multi-party P3. F-1's per-author event signature is not built (exo-f76), and the strip says so |
