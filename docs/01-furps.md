...
- **F-5** Replay binding: every signing payload commits to (a) execution environment id, (b) account address (for a room-native decision, the room), (c) serialization slot (nonce or nullifier), (d) expiry. A contribution is unusable in any other conversation, account, or slot.
- **F-6** Driver interface: `describe`, `canonicalize`, `verify`, `progress`, `assemble`, `submit`, `watch`. `describe` returns: policy shape (threshold), round count r, serialization domain, finality descriptor type. The core never interprets contribution bytes; it routes opaque blobs and asks the driver what they mean.
...
- **F-14** Identity: **two bound keypairs.** A **secp256k1 authorization identity** signs Safe transactions (Keycard is the hardware backend, v1). A separate **encryption/chat identity** — one Ed25519 key for authentication, with X25519 derived from it for key agreement, matching the platform chat/account layer — encrypts to members and authors room messages. The two are joined by an **authenticated binding**: the secp256k1 key signs a domain-separated link statement committing to the encryption public key, the account context, and an expiry (inv 2, inv 5), recorded as a first-class signed entry in the log. F-9's role intersection and epoch key-wrapping rest on this binding — **verified on ingest, never trusted**. Rationale (resolved at ADR-010, 2026-08-23, superseding the earlier one-curve ideal): key separation — an ECDSA signing key must not double as a static-ECDH decryption key — and the binding is precisely what consuming the platform's Ed25519/X25519 chat layer later requires, so building it now turns ADR-010's curve gap into a bridge rather than a blocker.
...
- **F-20** **Input provenance.** Every signing payload commits to a provenance record naming, for each input whose bytes reached the signed payload, its **class** — plugin block (F-12), driver-interpreted contribution (F-6), external read (FS-1), or peer message (F-1) — and its **log position**. The client **MUST refuse to sign when any input's origin cannot be accounted for**; an unaccountable input is a refusal, not a gap in the record. The record names the account that actually contributed each input; it stays inside the room's epoch (FS-7, F-16). This generalizes F-3's provenance references and F-10's per-fact verification grade from facts to every input on the signing path. F-4's re-derivation proves a materialization is consistent with its effect and says nothing about where the effect's own inputs came from, so correctly-derived data of unknown origin satisfies it.
...
- **FS-7** **Identity is scoped to the conversation.** Membership is **persistent and named within a room**: the group knows who it is talking to, and each member keeps a stable identity across the conversation. That identity lives in the room's encrypted interior — known to members, **never leaked to a store node or any observer outside the room**. Cross-room unlinkability by an outside observer is bounded only by the transport-metadata limit in FS-9 (which the platform mixnet closes when it ships); at the identity/content layer, an outsider learns nothing. Every driver is named: members see who approved, declined, or shared. Anonymous membership was retired (ADR-015); privacy here means private toward everyone **outside** the room. This is scoped to what the client controls and the room boundary; it does not cover what a store node observes at the transport layer (FS-9), and must never be cited as though it did.
- **FS-8** No telemetry, no phone-home, no automatic update fetch. Updates are manual or an explicitly enabled signed feed.
- **FS-9** **Metadata honesty.** The client must not claim protection it does not deliver. Concretely, under F-15 a store node observes which content topics a client subscribes to — its conversation set — and the timing of publishes and fetches. That is the conversation graph, pseudonymously, and end-to-end encryption does not hide it. Every surface describing what stays inside the room names this limit where it applies. Closing it requires a mixnet at the transport layer, not an application change — and the Logos stack is implementing a **general mixnet that routes all messages**, which Muster will route through when it ships (the same lean-on-native posture as delivery and chat: we consume the platform's transport privacy rather than building our own). Until it is available the limit is stated plainly rather than implied away. Running your own store node is not a fix — it protects the operator's own metadata, not that of the people they talk to, and can invert the threat.
- **FS-10** The provenance record of F-20 is **readable only inside the conversation**, scoped to the membership epoch (F-16) of the entries it describes: a member who joins at a later epoch cannot reconstruct earlier lineage, and an actor holding the log without an epoch's keys reconstructs none of that epoch's record. The record is a reduction over the log, never a side-car store — a provenance file written beside the log would rebuild identically on replay while handing a keyless reader the whole lineage. The refusal in F-20 cannot be disabled by configuration, plugin, or user preference, on the same footing as FS-6. Note the boundary this draws: a *participant* can always disclose lineage they legitimately hold, and F-20 does not attempt to prevent that — the conversation is the security boundary, so what FS-10 protects against is reading from **outside** it.
...
- **U-4** Approvals render as presence: slots fill as contributions arrive, each naming its member.
...
- **U-9** *Withdrawn (ADR-015).* Was: the anonymous-room nameplate. Every room is named; the scope sheet names who can read.
...
- **S-1** A driver conformance suite is the contract: shared fixtures covering rounds, serialization domains, thresholds, and finality. Every driver passes it in CI. No driver feature merges with conformance red.
- **S-2** Deterministic replay: any conversation log file replays to identical state. Log files are the bug-report format.
- **S-3** Structured local logging with levels; secrets never logged.
...
**Design.** No server. No plugin canvas; closed render vocabulary. Local-first. Settlement chain-agnostic; the app never becomes the settlement layer. The driver divergence axes — rounds, serialization domain, threshold, finality — are load-bearing and must not be collapsed for convenience.
