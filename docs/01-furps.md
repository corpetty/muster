...
- **FS-7** **Identity is scoped to the conversation.** The default membership model is **persistent and named within a room**: the group knows who it is talking to, and each member keeps a stable identity across the conversation. That identity lives in the room's encrypted interior — known to members, **never leaked to a store node or any observer outside the room**. Cross-room unlinkability by an outside observer is bounded only by the transport-metadata limit in FS-9 (which the platform mixnet closes when it ships); at the identity/content layer, an outsider learns nothing. Anonymous-membership drivers remain an **opt-in** (F-6's membership model is driver-described): under one, signer identity must additionally not leak through UI state, local logs, timing, or ordering **within the client** — slots render count-only. Either way this is scoped to what the client controls and the room boundary; it does not cover what a store node observes at the transport layer (FS-9), and must never be cited as though it did.
