...
#
# The module surface is generated from the LIDL contract (ADR-008) by the shared
# Nim SDK generator (../sdk/lidl-gen), NOT a muster-local copy — muster consumes
# logos_sdk. Until the builder runs this as part of its codegen step, run it by
# hand after editing muster.lidl. This wraps the multi-step recipe (build the LIDL
# C lib, build the generator against it, run it) so it is one command.
set -euo pipefail
cd "$(dirname "$0")/.."
...
echo "→ building the SDK generator (../sdk/lidl-gen/lidl_gen.nim)"
