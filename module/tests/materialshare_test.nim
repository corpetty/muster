...
## in provenance (F-20) — named under a named driver, a nonce under an anonymous one
## (invariant 9) — and discloses its public face to the room in the flow view (rule s6),
...
# ── 3. anonymous driver: the share names nobody (invariant 9) ─────────────────────
block:
  let events = @[proposeEvent(id, effectJson),
                 materialShareEvent(id, "payee", "nonce-xyz", "0xADDR", "public", "address", "to")]
  for it in logProvenance(events, anon):
    if it.kind == "material":
      doAssert it.account == "", "anonymous driver: the share names nobody"
  echo "3. anonymous driver: a share names nobody (inv 9) OK"

# ── 4. flow: a share discloses its public face to the ROOM; outside rows wait for submit ─
block:
  ...
  echo "4. flow: a share discloses its public face to the room; outside rows wait for submit OK"
