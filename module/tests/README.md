...
no external deps; `manifest_test` too). `decline_test` is pure Nim but links libsodium
...
Safe — `readiness_test` (the action-manifest readiness probe against a real SafeDriver, exo-002.2) and the crypto/coordination tests). We no longer link a system
