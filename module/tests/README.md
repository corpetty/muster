...
no external deps; `manifest_test` and `log_proof_test` too). `decline_test` / `provenance_all_test` / `flow_test` / `materialshare_test` (exo-45e K5) / `schema_unknown_test` (exo-1ec.3 — the schema-driven rendering gate) / `lez_readiness_test` (exo-44b L1) are pure Nim but link libsodium
