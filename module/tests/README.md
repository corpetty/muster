...
`binding_test`, `safe_owners_test` (exo-45e K3), `material_test` + `offers_test` + `lez_modeb_test` (exo-45e K2a/K4/ModeB — catalogue, offers, and the coordinated-transfer counterparty slot; need $STINT + libsodium), `conformance_test` — the driver conformance suite run against stub +
